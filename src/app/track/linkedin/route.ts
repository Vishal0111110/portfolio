...
async function sendLinkAccessEmail(linkData: any) {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      to: YOUR_EMAIL,
      subject: `🔗 LinkedIn Accessed - ${linkData.timestamp.toLocaleString()}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <h2 style="color: #333; border-bottom: 2px solid #0077b5; padding-bottom: 10px;">
            LinkedIn Profile Accessed
          </h2>
    ...
    console.log('LinkedIn access email sent successfully');
  } catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (firebaseDB) {
      try {
        await firebaseDB.collection('linkedin_accesses').add(linkData);
        console.log('LinkedIn access stored in Firebase Firestore');
      } catch (dbError) {
      ...
      console.log('Firebase not configured for LinkedIn tracking, skipping database storage');
      console.error('Firebase config check:', {
        hasProjectId: !!FIREBASE_PROJECT_ID,
        hasClientEmail: !!FIREBASE_CLIENT_EMAIL,
        hasPrivateKey: !!FIREBASE_PRIVATE_KEY,
    ...
    const linkedinUrl = 'https://www.linkedin.com/in/buyyarapuvishal/';
    return NextResponse.redirect(linkedinUrl);

  } catch (error) {
    console.error('Error tracking LinkedIn access:', error);
